...
for name in ['index.html','styles.css','design.js','app.js','engine.js','skills.js','config.js','data.js','data-auto.js','jobs.json','market-history.json',*(f'data-{i}.js' for i in range(1,7))]:
...
if os.getenv('SUPABASE_URL') and public_key:
    config={'apiBase':os.getenv('PUBLIC_API_BASE',''),'supabaseUrl':os.environ['SUPABASE_URL'],'supabaseAnonKey':public_key}
