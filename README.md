...
- APIs externas guardam descobertas em `job_candidates`. O fluxo `scripts/publish_candidates.py` só encaminha vagas de até 90 dias com anúncio atual no board cadastrado da própria empresa, cargo correspondente, requisitos e localização elegíveis. Após a validação e a sincronização do catálogo, vincula o candidato à vaga publicada. Anúncios antigos são rejeitados; casos sem fonte oficial rastreável ficam para revisão. O nome de um repositório do GitHub não identifica o empregador.
